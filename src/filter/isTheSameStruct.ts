const isSameVal = (defVal, structVal) => {
    if (typeof defVal === 'string') return defVal === structVal.type.name
...
export const isTheSameStruct = (def: {}, struct: {}): boolean => {
    ...
    if (defEntries.length !== structEntries.length) return false

    for (const e of defEntries) {
        const [name, val] = e
        const sVal = struct[name]
        if (sVal === undefined) return false

        if (!isSameVal(val, sVal)) return false
    }
